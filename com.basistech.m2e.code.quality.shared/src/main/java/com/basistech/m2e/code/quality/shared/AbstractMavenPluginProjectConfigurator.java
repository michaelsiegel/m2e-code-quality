 ...
 * 
 */
public abstract class AbstractMavenPluginProjectConfigurator extends AbstractProjectConfigurator {
  ...
  private boolean checkUnconfigurationRequired(IProgressMonitor monitor,
    ...
    if (curMavenProjectFacade == null || oldMavenProjectFacade == null) {
      return false;
    }
  ...
  protected MojoExecution findForkedExecution(MojoExecution primary, String groupId,
  ...
  public List<String> getCommaSeparatedStringParameterValues(String parameter,
    ...
    String value = this.getParameterValue(parameter, String.class, session, execution);
    if (value == null) {
  ...
  private MavenPluginWrapper getMavenPlugin(IProgressMonitor monitor,
    ...
    return MavenPluginWrapper.newInstance(monitor, this.getMavenPluginGroupId(),
        this.getMavenPluginArtifactId(), this.getMavenPluginGoal(), projectFacade);
  }
   ...
   * apply. Null may lead to chaotic overlaying of multiple configurations. If
   * more than one, this will process in order looking for an execution.
   * 
   * @return
   */
  ...
  @Override
  public <T> T getParameterValue(String parameter, Class<T> asType, MavenSession session,
  ...
  protected abstract void handleProjectConfigurationChange(final MavenSession session,
      final IMavenProjectFacade mavenProjectFacade, final IProject project,
      final IProgressMonitor monitor, final MavenPluginWrapper mavenPluginWrapper)
      throws CoreException;
  ...
  public void mavenProjectChanged(final MavenProjectChangedEvent mavenProjectChangedEvent,
    ...
    if (mavenProjectFacade != null) {
      ...
      if (pluginWrapper.isPluginConfigured()) {
        ...
        MavenSession session = this.maven.createSession(request, mavenProject);
        this.handleProjectConfigurationChange(session, mavenProjectFacade, project, monitor,
            pluginWrapper);
      } else {
