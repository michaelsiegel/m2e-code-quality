...
public class MavenPluginConfigurationTranslator {
  ...
  public static MavenPluginConfigurationTranslator newInstance(
      AbstractMavenPluginProjectConfigurator configurator, MavenSession session,
      final MavenProject mavenProject, final MavenPluginWrapper mavenPlugin, final IProject project)
      throws CoreException {
    final MavenPluginConfigurationTranslator m2csConverter = new MavenPluginConfigurationTranslator(
        configurator, session, mavenProject, mavenPlugin, project);
  ...
  private MavenPluginConfigurationTranslator(
      final AbstractMavenPluginProjectConfigurator configurator, final MavenSession session,
      final MavenProject mavenProject, final MavenPluginWrapper pluginWrapper,
  ...
  private String getConfigLocation() throws CoreException {
    String configLocation = this.configurator.getParameterValue("configLocation", String.class,
        this.session, this.execution);
    ...
    if (propertiesLocation != null) {
      ...
      if (url == null) {
        throw new CheckstylePluginException(String.format(
            "Failed to resolve propertiesLocation [%s]", propertiesLocation));
      }
      try {
        properties.load(url.openStream());
      } catch (IOException e) {
        throw new CheckstylePluginException(String.format(
            "Failed to LOAD properties from propertiesLocation [%s]", propertiesLocation));
      }
    }
    return properties;
  }
  ...
  private List<FileMatchPattern> getIncludesExcludesFileMatchPatterns()
     ...
     * enabled).
     */
    Set<String> sourceFolders = new HashSet<String>(this.mavenProject.getCompileSourceRoots());
     ...
     * NOTES: - Since the eclipse-cs excludes override (or more correctly the
     * later ones) the includes, we make sure that if no include patterns are
     * specified we at least put the source folders. - Also, we use relative
     * path to project root.
     */
    final List<String> includePatterns = this.getIncludes();
    for (String folder : sourceFolders) {
      final String folderRelativePath = this.basedirUri.relativize(new File(folder).toURI())
          .getPath();
      if (includePatterns.size() != 0) {
        patterns.addAll(this.normalizePatternsToCheckstyleFileMatchPattern(includePatterns,
     ...
     * NOTES: - Since the eclipse-cs excludes override (or more correctly the
     * later ones) the includes, we do NOT add the sourceFolder to exclude list.
     */
    final List<String> excludePatterns = this.getExcludes();
    for (String folder : sourceFolders) {
      String folderRelativePath = this.basedirUri.relativize(new File(folder).toURI()).getPath();
      patterns.addAll(this.normalizePatternsToCheckstyleFileMatchPattern(excludePatterns,
          this.convertToEclipseCheckstyleRegExpPath(folderRelativePath), false));
    }
  ...
  public boolean getIncludeTestSourceDirectory() throws CoreException {
    Boolean includeTestSourceDirectory = this.configurator.getParameterValue(
        "includeTestSourceDirectory", Boolean.class, this.session, this.execution);
  ...
  private List<String> getPatterns(String elemName) throws CoreException {
    ...
    final String patternsString = this.configurator.getParameterValue(elemName, String.class,
        this.session, this.execution);
  ...
  protected String getPropertiesLocation() throws CoreException {
    return this.configurator.getParameterValue("propertiesLocation", String.class, this.session,
        this.execution);
  }
  ...
  protected String getPropertyExpansion() throws CoreException {
    return this.configurator.getParameterValue("propertyExpansion", String.class, this.session,
        this.execution);
  }

  public URL getRuleset() throws CheckstylePluginException, CoreException {
    final URL ruleset = this.resourceResolver.resolveLocation(this.getConfigLocation());
    if (ruleset == null) {
      throw new CheckstylePluginException(
          String
              .format("Failed to resolve RuleSet from configLocation,SKIPPING Eclipse checkstyle configuration"));
    }
    return ruleset;
  }

  private List<FileMatchPattern> normalizePatternsToCheckstyleFileMatchPattern(
      final List<String> patterns, final String relativePath, final boolean setIsIncludePatternFlag)
      throws CheckstylePluginException {
    List<FileMatchPattern> fileMatchPatterns = new LinkedList<FileMatchPattern>();
    for (String p : patterns) {
      final FileMatchPattern fmp = new FileMatchPattern(String.format("%s%s", relativePath, p));
    ...
    if (propertyExpansion != null) {
      try {
        ...
        throw new CheckstylePluginException(String.format(
            ...
            CheckstyleEclipseConstants.LOG_PREFIX, propertyExpansion));
      }
    }
  }
